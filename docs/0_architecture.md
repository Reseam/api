...
![Architecture diagram: the Reseam API fetches and caches one configured patches.json from reseam/patches, serves /v1/patches and /patches/:tag/:name to Reseam Manager and the website, and stores announcements in SQLite. Third-party bundles bypass the API; Reseam Manager reads their patches.json URLs directly.](architecture.svg)
