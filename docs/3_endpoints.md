...
`bundle` is the upstream `bundle` block. `release` is one entry from the upstream `releases` array. Patch releases include the catalog generated by their publisher; manager releases omit it. Use the structured endpoints when you want typed data and the raw indexes when you need the publisher's JSON unchanged.
